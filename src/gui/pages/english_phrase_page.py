...
from PyQt6.QtWidgets import (
    ...
    QTableWidgetItem, QHeaderView, QLineEdit, QSizePolicy, QStackedWidget
)
from PyQt6.QtGui import QFont, QPixmap

from .base_page import BasePage
from ..fonts import mi
from core.english_parser import parse_english_to_regex
from core.preprocessor import expand
...
class EnglishPhrasePage(BasePage):

    def __init__(self, parent=None):
        super().__init__(title="English Phrase", icon=mi.EXPLORE,
                         subtitle="// Natural language pattern module", parent=parent)
    ...
    def _build_input_panel(self) -> None:
        ...
        grid_lay.addWidget(QLabel("Alphabet:"), 0, 0)
        ...
        self._alpha_input.setStyleSheet(
            "QLineEdit { background:#F5F0E8; border:2px solid #111111; padding:8px;"
            " font-family:'Consolas'; font-weight:700; font-size:14px; }"
        )
        grid_lay.addWidget(self._alpha_input, 0, 1)

        grid_lay.addWidget(QLabel("Phrase:"), 1, 0)
        self._phrase_input = QLineEdit()
        self._phrase_input.setPlaceholderText("e.g. starts with a and ends with b")
        self._phrase_input.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        examples = QLabel("Examples: 'contains exactly 2 a', 'ends with b', 'does not contain a'")
        examples.setStyleSheet("color: #888888; font-size: 11px; font-weight: 700; margin-top: 10px;")
    ...
    def _evaluate(self) -> None:
        ...
        if not phrase:
            self._show_error("Please enter a phrase.")
            return

        raw_alpha = self._alpha_input.text().strip()
        alphabet = [s for s in raw_alpha.split() if s]
        if not alphabet:
            alphabet = ["a", "b"]
        ...
        self._results_expr_label.setText(f"PHRASE  →  REGEX: {regex_str}")
    ...
    def _build_regex_tab(self) -> None:
        ...
        note = QLabel("Mathematically derived from natural language phrase.")
    ...
    def _populate_dfa_tab(self, dfa) -> None:
        img_path = os.path.join(self._temp_dir, "en_dfa_inp")
        try:
    ...
    def _populate_min_dfa_tab(self, min_dfa) -> None:
        img_path = os.path.join(self._temp_dir, "en_min_dfa_inp")
        try:
    ...
    def _reset_sim_tab(self) -> None:
        ...
        self._sim_steps = []
        if self._min_dfa:
            img_path = os.path.join(self._temp_dir, "en_sim_dfa_inp")
            try:
    ...
    def _start_sim(self) -> None:
        ...
        img_path = os.path.join(self._temp_dir, "en_sim_step_init_inp")
        try:
    ...
    def _sim_step(self) -> None:
        ...
        img_path = os.path.join(self._temp_dir, f"en_sim_step_inp_{self._sim_current_step}")
        try:
            if step["dead"]:
